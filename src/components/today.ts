...
export class Today {
  ...
  constructor(data: TodayElem, map: Map<number, string>) {
    this.cityElem = document.getElementById("city")! as HTMLElement;
    this.dateElem = document.getElementById("date")! as HTMLElement;
    this.currentTempElem = document.getElementById(
      "current-temp"
    )! as HTMLElement;
    this.todayIconElem = document.getElementById(
      "today-weather-icon"
    )! as HTMLElement;
    ...
    this.updateUI(data, map);
  }

  updateUI(data: TodayElem, map: Map<number, string>) {
    ...
    this.todayIconElem.innerHTML = `<img src=${map.get(data.code)} alt="rain">`;
  }
}
